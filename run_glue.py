...
def main():
    ...
    encoder, predictor, _, _, _, _ = load_checkpoint(
        device=training_args.device,
        r_path=model_args.model_name_or_path,
        encoder=encoder,
        predictor=predictor,
        target_encoder=None,
        opt=None,
    ...
    class MyT5ClassificationHead(torch.nn.Module):
        ...
        def __init__(self, d_model, n_labels, classifier_dropout=0.0):
            super().__init__()
            self.dense = torch.nn.Linear(d_model, d_model)
            self.dropout = torch.nn.Dropout(p=classifier_dropout)
            self.out_proj = torch.nn.Linear(d_model, n_labels)
    ...
    class MeanPooledEncoder(torch.nn.Module):
        ...
        def forward(self,
            ...
            assert len(sequence_output.shape) == 3, f'Expected sentence_output to have shape length 3, got shape {input_ids.shape}!'
            batch_size, _, hidden_size = sequence_output.shape
            sentence_representation = self.encoder(input_ids).mean(dim=1)
            assert len(sentence_representation.shape) == 2
